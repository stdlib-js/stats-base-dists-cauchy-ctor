...
function Cauchy() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( x0 ) || isnan( x0 ) ) {
			throw new TypeError( format( '0wN8p,HM', x0 ) );
		}
		if ( !isPositive( gamma ) ) {
			throw new TypeError( format( '0wN7c,HN', gamma ) );
		}
	} else {
		x0 = 0.0;
		gamma = 1.0;
	...
	defineProperty( this, 'x0', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '0wN8d,H8', value ) );
			}
			x0 = value;
		}
	});
	defineProperty( this, 'gamma', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '0wN8k,HE', value ) );
			}
			gamma = value;
		}
	});
